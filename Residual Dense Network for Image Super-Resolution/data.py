...
def getPatch(imgIn, imgTar, args, scale):
    ...
    tp = args.patch_size # HR image patch size
    ip = tp // scale
...
def RGB_np2Tensor_test(imgIn, imgTar, channel):
    if channel == 1:
        # rgb --> Y (gray)
        imgTar_Cb = np.sum(imgIn * np.reshape([-37.797, - 74.203, 112.000], [1, 1, 3]) / 255.0,
                           axis=2, keepdims=True) + 128.0
        imgTar_Cr = np.sum(imgIn * np.reshape([112.000, - 93.786, - 18.214], [1, 1, 3]) / 255.0,
                           axis=2, keepdims=True) + 128.0
        imgIn = np.sum(imgIn * np.reshape([65.481, 128.553, 24.966], [1, 1, 3]) / 255.0, axis=2, keepdims=True) + 16.0
        imgTar = np.sum(imgTar * np.reshape([65.481, 128.553, 24.966], [1, 1, 3]) / 255.0, axis=2, keepdims=True) + 16.0
    ...
    imgIn = torch.Tensor(imgIn.transpose(ts).astype(float)).mul_(1.0)
    imgTar = torch.Tensor(imgTar.transpose(ts).astype(float)).mul_(1.0)
    imgTar_Cb = torch.Tensor(imgTar_Cb.transpose(ts).astype(float)).mul_(1.0)
    imgTar_Cr = torch.Tensor(imgTar_Cr.transpose(ts).astype(float)).mul_(1.0)

    # normalization [-1,1]
    imgIn = (imgIn / 255.0 - 0.5) * 2
    imgTar = (imgTar / 255.0 - 0.5) * 2
    return imgIn, imgTar, imgTar_Cb, imgTar_Cr
...
class Set5_test(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return RGB_np2Tensor_test(imgIn, imgTar, self.channel)
